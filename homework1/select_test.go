...
func TestSelector_Having(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "alias having",
			q:    NewSelector[TestModel](db).Select(Avg("Id").As("testId")).GroupBy(C("Age")).Having(C("Id").LT(20)),
			wantQuery: &Query{
				SQL:  "SELECT AVG(`id`) AS `testId` FROM `test_model` GROUP BY `age` HAVING `testId` < ?;",
				Args: []any{20},
			},
		},
	}
