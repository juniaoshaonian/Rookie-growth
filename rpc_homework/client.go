...
type Client struct {
	...
	compresser compress.Compresser
}
...
func (c *Client) InitService(val Service) error {
	return setFuncField(c.serializer, val, c, c.compresser)
}

// 这个单独的拆出来，就是为了测试，我们可以考虑传入一个 mock proxy
func setFuncField(s serialize.Serializer, val Service, proxy Proxy, c compress.Compresser) error {
